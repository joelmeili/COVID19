...
server <- function(input, output) {
    ...
    output$plot1 <- renderPlot({
        get_df() %>% filter(`Actual/Simulated` == "Actual" & SIR == "Infected") %>% na.omit() %>% ggplot(aes(x = date, y = Cases)) + geom_line() + 
            theme_base() + xlab("Date") + ylab("# of Infections") + ggtitle("Number of infections per 100'000")
    })
    
    output$plot2 <- renderPlot({
        grid.arrange(
            df %>% filter(`Actual/Simulated` == "Actual" & SIR == "Infected") %>% na.omit() %>% ggplot(aes(x = date, y = Cases, colour = `Country/Region`)) + 
    ...
    output$plot5 <- renderPlot({
        df %>% filter(SIR == "Infected" & `Actual/Simulated` == "Simulated") %>% ggplot(aes(x = date, y = Cases, colour = `Country/Region`)) + geom_line() +
            scale_y_log10() + theme_base() + xlab("Date") + ylab("# of Cases") + ggtitle("Simulated # of infected cases across countries per 100'000")
    })
}
